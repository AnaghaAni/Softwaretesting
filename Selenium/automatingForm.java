...
public class automatingForm {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Thread.sleep(2000);
		radioFemale.click(); // Click the label (this triggers the associated hidden <input> radio button)
		...
		// 		These inputs use dynamic dropdowns, so direct typing and ENTER key are used.
		...
		// Print the list of dropdown elements
		...
		for (WebElement state : stateDropDown) {
		    ...
		    if (selectedState.equals("Rajasthan")) {
		        js.executeScript("arguments[0].scrollIntoView(true)", state); // Scroll to it
		        state.click(); // Select the state
		        break;
		    }
		}
		...
		for (WebElement city : cityDropDown) {
		    ...
		    if (selectedcity.equals("Jaipur")) {
		        js.executeScript("arguments[0].scrollIntoView(true)", city); // Scroll to it
		        city.click(); // Select the city
		        break;
		    }
		}

		// Combine state and city and store in map for verification
		...
		// Submitting the form and validating the displayed table data with expected input
		...
		// Create a LinkedHashMap to store key-value pairs from the table
		...
		// Print the full map of actual table data
		//System.out.println(actualData);

		// Compare actual vs expected data stored 
		for (String key1 : expectedData.keySet()) {
		    String expectedValue = expectedData.get(key1);  // Get expected value
		    String actualValue = actualData.get(key1);      // Get actual value
		    // Match check
